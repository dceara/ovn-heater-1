...
OS_TYPE="${OS_TYPE:?Must set OS_TYPE (fedora or ubuntu)}"
...
cmd_run() {
    ...
    # Use the VM's known IP for the CI host address.  The hostname
    # resolves to 127.0.0.1 inside containers which is their own
    # loopback, not the VM host.
    ...
    # Restore runtime cache if available (container images, built
    # sources).
