...
class Namespace:
    ...
    @ovn_stats.timeit
    def add_ports(self, ports, cluster_id=0):
        self.ports[cluster_id].extend(ports)
        ...
        if self.enforcing:
            self.nbctl[cluster_id].port_group_add_ports(
                self.pg_def_deny_igr[cluster_id], ports
            )
            self.nbctl[cluster_id].port_group_add_ports(
                self.pg_def_deny_egr[cluster_id], ports
            )
            self.nbctl[cluster_id].port_group_add_ports(
                self.pg[cluster_id], ports
            )
    ...
    def unprovision_ports(self, ports, cluster_id=0):
        ...
        for port in ports:
            self.ports[cluster_id].remove(port)

        self.clusters[cluster_id].unprovision_ports(ports)
    ...
    @ovn_stats.timeit
    def default_deny(self, family, cluster_id=0):
        self.enforce()

        addr_set = f'self.addr_set{family}.name'
        self.nbctl[cluster_id].acl_add(
            self.pg_def_deny_igr[cluster_id].name,
            'to-lport',
            ACL_DEFAULT_DENY_PRIO,
            'port-group',
            f'ip4.src == \\${addr_set} && '
            f'outport == @{self.pg_def_deny_igr[cluster_id].name}',
            'drop',
        )
        self.nbctl[cluster_id].acl_add(
            self.pg_def_deny_egr[cluster_id].name,
            'to-lport',
            ACL_DEFAULT_DENY_PRIO,
            'port-group',
            f'ip4.dst == \\${addr_set} && '
            f'inport == @{self.pg_def_deny_egr[cluster_id].name}',
            'drop',
        )
        self.nbctl[cluster_id].acl_add(
            self.pg_def_deny_igr[cluster_id].name,
            ...
            'port-group',
            f'outport == @{self.pg_def_deny_igr[cluster_id].name} && arp',
            'allow',
        )
        self.nbctl[cluster_id].acl_add(
            self.pg_def_deny_egr[cluster_id].name,
            ...
            ACL_DEFAULT_ALLOW_ARP_PRIO,
            'port-group',
            f'inport == @{self.pg_def_deny_egr[cluster_id].name} && arp',
            'allow',
        )

    @ovn_stats.timeit
    def allow_within_namespace(self, family, cluster_id=0):
        self.enforce()

        addr_set = f'self.addr_set{family}.name'
        self.nbctl[cluster_id].acl_add(
            self.pg[cluster_id].name,
            'to-lport',
            ACL_NETPOL_ALLOW_PRIO,
            'port-group',
            f'ip4.src == \\${addr_set} && '
            f'outport == @{self.pg[cluster_id].name}',
            'allow-related',
        )
        self.nbctl[cluster_id].acl_add(
            self.pg[cluster_id].name,
            ...
            'port-group',
            f'ip4.dst == \\${addr_set} && '
            f'inport == @{self.pg[cluster_id].name}',
            'allow-related',
        )

    @ovn_stats.timeit
    def allow_cross_namespace(self, ns, family, cluster_id=0):
        self.enforce()
        ...
        self.nbctl[cluster_id].acl_add(
            ...
            'port-group',
            f'ip4.src == \\${addr_set} && '
            f'outport == @{ns.pg[cluster_id].name}',
            'allow-related',
        )
    ...
    @ovn_stats.timeit
    def allow_sub_namespace(self, src, dst, family, cluster_id=0):
        self.nbctl[cluster_id].acl_add(
            self.pg[cluster_id].name,
            'to-lport',
            ACL_NETPOL_ALLOW_PRIO,
    ...
    def allow_from_external(
        self, external_ips, include_ext_gw=False, family=4, cluster_id=0
    ):
        self.enforce()
        ...
        if include_ext_gw:
            ...
            if family == 4 and self.ports[cluster_id][0].ext_gw:
                external_ips.append(self.ports[cluster_id][0].ext_gw)
            elif family == 6 and self.ports[cluster_id][0].ext_gw6:
                external_ips.append(self.ports[cluster_id][0].ext_gw6)
    ...
    @ovn_stats.timeit
    def check_enforcing_internal(self, cluster_id=0):
        # "Random" check that first pod can reach last pod in the namespace.
        if len(self.ports) > 1:
            src = self.ports[cluster_id][0]
            dst = self.ports[cluster_id][-1]
            worker = src.metadata
            if src.ip:
                worker.ping_port(self.clusters[cluster_id], src, dst.ip)
            if src.ip6:
                worker.ping_port(self.clusters[cluster_id], src, dst.ip6)

    @ovn_stats.timeit
    def check_enforcing_external(self, cluster_id=0):
        if len(self.ports) > 0:
            dst = self.ports[cluster_id][0]
            worker = dst.metadata
            worker.ping_external(self.clusters[cluster_id], dst)

    @ovn_stats.timeit
    def check_enforcing_cross_ns(self, ns, cluster_id=0):
        if len(self.ports) > 0 and len(ns.ports) > 0:
            dst = ns.ports[cluster_id][0]
            src = self.ports[cluster_id][0]
            worker = src.metadata
    ...
    def create_load_balancer(self, cluster_id=0):
        self.load_balancer = lb.OvnLoadBalancer(
            f'lb_{self.name}', self.nbctl[cluster_id]
        )

    @ovn_stats.timeit
    def provision_vips_to_load_balancers(
        self, backend_lists, version, cluster_id=0
    ):
